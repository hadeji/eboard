...
class CustomersDB extends DB\SQL\Mapper {
    ...
    public function check_customer_by_email($email){
    	try{
    		$this->load(array('emailAddress=?', $email));
            return $this->query;
    ...
    public function register_account($fname, $lname, $uname, $email, $pass, $bdate, $sbaddress){
        try{
            $this->reset();
            ...
            $this->shippingBillingAddress = $sbaddress;
            $this->save();
        }catch(PDOException $e){
            echo $e->getMessage();
        }
    ...
    public function get_customer_by_username($uname){
        $f3 = Base::instance();
        $db = $f3->get('DB');
        try{
